...
queries = [
    "CREATE OR REPLACE FUNCTION get_auth_user_id() RETURNS int LANGUAGE sql STABLE AS $$ SELECT id FROM users WHERE supabase_id = auth.uid()::text LIMIT 1; $$;",
    ...
    "CREATE POLICY select_own_profile ON profiles FOR SELECT USING (user_id = get_auth_user_id());",
    "CREATE POLICY update_own_profile ON profiles FOR UPDATE USING (user_id = get_auth_user_id());",
    "CREATE POLICY insert_own_profile ON profiles FOR INSERT WITH CHECK (user_id = get_auth_user_id());",
    "CREATE POLICY manage_own_vault_items ON vault_items FOR ALL USING (user_id = get_auth_user_id());",
